get_canopy <- function(canopy_files, parks){
  ...
  cancov <- aggregate(canopy_files, st_transform(relevant_parks, st_crs(canopy_files)), FUN = function(x) sum(x == 4, na.rm = T)/length(x)) %>% 
    st_as_sf()
